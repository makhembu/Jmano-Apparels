...
export const PaymentOptions: React.FC<PaymentOptionsProps> = ({
  ...
  const handlePayPalError = (err: any) => {
    // Ignore validation errors thrown intentionally from createOrder
    const errString = err?.toString() || '';
    if (errString.includes("Invalid order data")) {
        return;
    }
    
    console.error("PayPal Button Error:", err);
    setPaypalError(true);
    // If it's a database error (like constraint violation), we might see it in the error object structure depending on how it bubbled up.
    // Usually PayPal SDK wraps it. We try to be helpful.
    setErrorMessage("Payment Gateway Unavailable");
    showToast("Payment Gateway Unavailable. Check console for details.", "error");
  };
