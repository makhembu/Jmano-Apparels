...
-- 1. Add Guest Checkout Toggle to App Settings
ALTER TABLE public.app_settings 
ADD COLUMN IF NOT EXISTS require_login_for_checkout BOOLEAN DEFAULT false;

-- 2. Modify Orders Table to support Guests
...
-- 3. Update the create_order_secure RPC to handle guests
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
     ...
     -- Fallback if not found in public users but exists in auth (rare)
  ...
  ELSE
     ...
     IF v_final_email IS NULL OR trim(v_final_email) = '' THEN
