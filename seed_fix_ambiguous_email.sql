...
-- ============================================================================
-- FIX: AMBIGUOUS EMAIL COLUMN IN CHECKOUT
-- Resolves the "column reference 'email' is ambiguous" error by specifying
-- the table alias in the user lookup query.
-- ============================================================================
...
BEGIN
  ...
  IF p_user_id IS NOT NULL THEN
     -- FIX: Explicitly select columns from public.users to avoid ambiguity with auth.users
     SELECT public.users.email, public.users.name 
     INTO v_final_email, v_final_name 
     FROM public.users 
     WHERE public.users.id = p_user_id;
     
     -- Fallback if not found (shouldn't happen for valid users)
  ...
  LOOP
    SELECT price, weight, stock_quantity, is_free_shipping INTO v_product_price, v_product_weight, v_product_stock, v_is_free_ship
    ...
    -- Only add weight if product is NOT free shipping
  ...
  -- Apply Global Free Shipping Threshold OR if weight calculation results in cost
