    ...
    <div className="animate-fade-in bg-slate-50">
        ...
        <div className="bg-white py-16">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-brand-dark mb-4 font-serif">Our Commitment to Faith & Quality</h2>
              ...
              <div className="w-20 h-1.5 bg-brand-hope rounded-full mt-8 mx-auto"></div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              {(settings.coreValues || 'Honesty,Excellence,Boldness').replace(/\{.*\}/, '').split(',').map((value, index) => {
                ...
                return (
                  <div key={index} className={`${detail.bg} ${detail.textColor} p-8 rounded-3xl shadow-xl text-center flex flex-col items-center`}>
                    <div className={`w-16 h-16 ${detail.iconBg} rounded-2xl flex items-center justify-center font-serif font-bold text-3xl mb-6`}>{letter}</div>
      ...
      {(settings.enableCommunitySection ?? true) && !showSkeletons && latestReviews.length > 0 && (
        <div className="py-16 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center max-w-2xl mx-auto mb-12">
              <h2 className="text-3xl font-bold font-serif text-brand-dark mb-4">From Our Community</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {latestReviews.slice(0, 3).map((review) => (
                <div key={review.id} className="bg-slate-50 border border-slate-200 rounded-2xl p-8 flex flex-col text-center shadow-sm">
                  <div className="text-amber-400 text-2xl mb-4">{'★'.repeat(review.rating)}<span className="text-slate-300">{'★'.repeat(5 - review.rating)}</span></div>
      ...
      {(settings.enableSocialSection ?? true) && !showSkeletons && settings.socialLinks && (
        <div className="bg-brand-dark py-16">
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <h2 className="text-3xl font-bold font-serif text-white mb-4">Follow Our Journey</h2>
            ...
            <div className="flex justify-center space-x-6">
              {Object.entries(settings.socialLinks).map(([platform, url]) =>
                url && SocialIcons[platform] ? (
                  ...
                  >
                    {/* FIX: Corrected cloneElement usage by adding className to the source SVG elements. */}
