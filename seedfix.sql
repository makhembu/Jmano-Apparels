-- 1. Correct the tax rate in app_settings (0.20 = 20%)
UPDATE public.app_settings SET tax_rate = 0.20 WHERE id = 1;

-- 2. Update the Order RPC to use dynamic tax rate from settings
...
BEGIN
  ...
  -- If rate was stored as whole number (e.g. 20), normalize to decimal
  ...
  -- 4. Calculate Tax (Inclusive of Price)
  -- Formula: (Total / (1 + Rate)) * Rate
