-- This script fixes the stock decrement logic in the create_order_secure function
-- and cleans up any existing products with NULL stock.

-- 1. Initialize any NULL stock or sales columns to 0 for existing products
UPDATE public.products
SET
  stock_quantity = COALESCE(stock_quantity, 0),
  total_sales = COALESCE(total_sales, 0)
WHERE stock_quantity IS NULL OR total_sales IS NULL;


-- 2. Update the create_order_secure function to correctly handle NULL stock and decrement it.
...
BEGIN
  ...
  -- 1. Calculate Subtotal and Weight, and check stock
  -- FIX: Using the full, correct record definition for items to ensure robust parsing.
  ...
  LOOP
    ...
    -- FIX: Use COALESCE to handle NULL stock values
